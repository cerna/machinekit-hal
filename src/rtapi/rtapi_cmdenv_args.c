/********************************************************************
* Description:  rtapi_cmdenv_args.c
*               This file, 'rtapi_cmdenv_args.c', implements functions
...
*               the 'ps' command and manipulating the environment variable set
*
...
#include "rtapi_cmdenv_args.h"
#include "rtapi_atomics.h"
#include "syslog_async.h"
#include "hal_types.h"

// TEMPORARY SOLUTION
// Test if syslog logging is possible and if not, then log to the console
#define syslog_async(level, ...)          \
    if (async_log_open())                 \
    {                                     \
        syslog_async(level, __VA_ARGS__); \
    }                                     \
    else                                  \
    {                                     \
...
static int current_state_argc = 0;
...
static hal_u8_t cmdenv_args_state = 0;
#define CMDENV_ARGS_STATE_INIT 0b1
#define CMDENV_ARGS_STATE_EXIT 0b10
#define CMDENV_ARGS_STATE_INITIALIZED CMDENV_ARGS_STATE_INIT
#define CMDENV_ARGS_STATE_EXITED CMDENV_ARGS_STATE_INIT | CMDENV_ARGS_STATE_EXIT
...
static pthread_mutex_t cmdenv_mutex = PTHREAD_MUTEX_INITIALIZER;

static void rtapi_cmdenv_args_exit(void)
{
    pthread_mutex_lock(&cmdenv_mutex);
    if (rtapi_cas_u8(&cmdenv_args_state, CMDENV_ARGS_STATE_INITIALIZED, CMDENV_ARGS_STATE_EXITED))
    {
        free(current_state_argv);
        free(new_environ);
        free(new_environ_space);
    }
    pthread_mutex_unlock(&cmdenv_mutex);
}
...
*/
static int rtapi_cmdenv_args_init(void)
{
    int fd_stat = -1;
    int retval = -1;
    ...
    syslog_async(LOG_ERR, "RTAPI: RTAPI_CMDENV_INIT: Minimal Linux Kernel version allowed is 3.5\n");
    exit(EXIT_FAILURE);
#endif
    ...
    if (!(rtapi_cas_u8(&cmdenv_args_state, 0, CMDENV_ARGS_STATE_INITIALIZED)))
    {
        hal_u8_t temp_state = rtapi_load_u8(&cmdenv_args_state);
        if (temp_state & CMDENV_ARGS_STATE_EXIT)
        {
            // Cannot be called after exit state, should not ever happen
            syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT was called after the STATE was changed to EXITED\n");
            retval = EPERM;
            goto return_value;
        }
        if (temp_state & CMDENV_ARGS_STATE_INIT)
        {
            // Nothing to do, the function can be called only once, should not ever happen
            syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT already done\n");
            retval = ENOENT;
            goto return_value;
        }
    }
    ...
    if (fd_stat < 0)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not open /proc/%d/stat\n", getpid());
        retval = ENODEV;
        goto error_end;
    }
    ...
    if (retval)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not close /proc/%d/stat, fd: %d\n", getpid(), fd_stat);
        // This error is not good, but not really something which prevents the rest of execution
    }
    if (read_characters < 1)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not read /proc/%d/stat\n", getpid());
        retval = EINVAL;
        goto error_end;
    }
    ...
    new_environ = calloc((envc + 1), sizeof(char *));
    if (!new_environ)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not malloc memory for environment vector\n");
        retval = errno;
        goto error_end;
    }
    new_environ_space = calloc((size_t)(((char *)old_env_end - (char *)old_env_start) + 1), 1);
    if (!new_environ_space)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not malloc memory for new environment space\n");
        retval = errno;
        goto error_environ_space_malloc;
    }

    // We need to compute number of strings in memory block
    temporary_character = (char *)old_arg_start;
    for (int shift = 0; temporary_character < (char *)old_arg_end;)
    {
        shift = strlen(temporary_character);
        if (shift > 0)
        {
            current_state_argc++;
            temporary_character = (char *)(temporary_character + shift + 1);
        }
        else
        {
            temporary_character++;
    ...
    current_state_argv = calloc((current_state_argc + 1), sizeof(char *));
    if (!current_state_argv)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not malloc memory for new argument vector\n");
        retval = errno;
        goto error_argv_malloc;
    }

    temporary_character = (char *)old_arg_start;
    for (int shift = 0, index = 0; (temporary_character < (char *)old_arg_end) && (index < current_state_argc); index++)
    {
        shift = strlen(temporary_character);
        if (shift > 0)
        {
            current_state_argv[index] = temporary_character;
            temporary_character = (char *)(temporary_character + shift + 1);
        }
        else
        {
    ...
    {
        size_t delta = environ[i] - environ[0];
        new_environ[i] = (char *)(temporary_character + delta);
    }
    ...
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not PR_SET_MM_MAP\n");
        retval = errno;
    ...
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not PR_SET_MM_ENV_START\n");
        retval = errno;
        goto error_pr_set_mm;
    }
    ...
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not PR_SET_MM_ENV_END\n");
        retval = errno;
        goto error_pr_set_mm;
    }
    if (prctl(PR_SET_MM, PR_SET_MM_ARG_START, (unsigned long int)data_first_stake, 0, 0) < 0)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not PR_SET_MM_ARG_START\n");
        retval = errno;
        goto error_pr_set_mm;
    }
    if (prctl(PR_SET_MM, PR_SET_MM_ARG_END, (unsigned long int)data_last_stake, 0, 0) < 0)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not PR_SET_MM_ARG_END\n");
        retval = errno;
        goto error_pr_set_mm;
    }
#endif
    ...
    retval = atexit(rtapi_cmdenv_args_exit);
    if (retval)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not register ATEXIT function\n");
...
error_process_stat:
    syslog_async(LOG_ERR, "RTAPI_CMDENV_INIT could not process /proc/%d/stat\n", getpid());
    retval = EINVAL;
    goto error_end;
error_pr_set_mm:
    free(current_state_argv);
...
error_environ_space_malloc:
    free(new_environ);
error_end:
    rtapi_and_u8(&cmdenv_args_state, ~CMDENV_ARGS_STATE_INIT);
return_value:
    return -retval;
}
...
const char *const rtapi_get_process_name(void)
{
    ...
    if (!(temp_state & CMDENV_ARGS_STATE_INIT))
    {
        ...
        {
            syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS GET_PROCESS_NAME could not return the current process name because an error occurred.\n");
            return NULL;
        }
    }
    if (temp_state & CMDENV_ARGS_STATE_EXIT)
    {
        syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS GET_PROCESS_NAME has been called after exit\n");
        return NULL;
    }
...
bool rtapi_set_process_name(const char *const new_name)
{
    ...
    char main_thread_name_path[PATH_MAX] = {0};
    char *temporary_space = NULL;
    char *delimiter = NULL;
    int main_threan_name_fd = -1;
    bool retval = false;
    ...
    if (!(temp_state & CMDENV_ARGS_STATE_INIT))
    {
        if (rtapi_cmdenv_args_init())
        {
            syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS SET_PROCESS_NAME could not set the current process name because an error occurred.\n");
            return false;
        }
    }
    if (temp_state & CMDENV_ARGS_STATE_EXIT)
    ...
    {
        int error = errno;
        syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS SET_PROCESS_NAME encountered and error (%d)-> %s when trying to allocate %ld chars for temporary_space\n", error, strerror(error), used_data_counter);
        goto mutex_release;
    }
...
int rtapi_execute_on_cmdline_copy(cmdline_data_callback cmdline_process_function, void *cloobj)
{
    ...
    if (!(temp_state & CMDENV_ARGS_STATE_INIT))
    {
        retval = rtapi_cmdenv_args_init();
        if (retval)
        {
            syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS EXECUTE_ON_CMDLINE_COPY could not execute because an error occurred.\n");
            goto end;
        }
    }
    if (temp_state & CMDENV_ARGS_STATE_EXIT)
    ...
    if (!temporary_space)
    {
        pthread_mutex_unlock(&cmdenv_mutex);
        retval = -errno;
        syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS EXECUTE_ON_CMDLINE_COPY encountered and error (%d)-> %s when trying to allocate %ld chars for temporary_space\n", retval, strerror(retval), used_data_counter);
        goto end;
    }
    ...
    if (!temporary_argv)
    {
        pthread_mutex_unlock(&cmdenv_mutex);
        retval = -errno;
        syslog_async(LOG_ERR, "RTAPI_CMDENV_ARGS EXECUTE_ON_CMDLINE_COPY encountered and error (%d)-> %s when trying to allocate %ld chars for temporary_argv\n", retval, strerror(retval), current_state_argc * sizeof(char *));
        goto temporary_space_free;
    }
