...
add_architecture_to_existing_entry(){
    local REGEX="^(deb(-src){0,1}[[:space:]]\[arch=.{1,})\][[:space:]][^ ]{0,}$3[^ ]{0,}[[:space:]].+$"
    local OUTPUT=""
    local LINE=""
...
copy_and_transform_repositories(){
    local REGEX="^deb(-src){0,1}[[:space:]]\[arch=(.{1,})\][[:space:]][^ ]{0,}$2[^ ]{0,}[[:space:]].+$"
    local OUTPUT=""
    local LINE=""
...
lock_to_architecture(){
    local REGEX="^(deb(-src){0,1}[[:space:]])[^ ]{0,}$3[^ ]{0,}[[:space:]].+$"
    local OUTPUT=""
    local LINE=""
...
attend_to_ubuntu(){
    ...
    then
        ...
        case "$HOST_ARCHITECTURE" in
            amd64 | i386)
                local address_to="ubuntu.com/ubuntu"
            ;;
            *)
                local address_to="ports.ubuntu.com/ubuntu-ports"
            ;;
        esac
