/********************************************************************
* Description:  posix-common.c
*
*               This file, 'posix-common.c', implements the unique common
*               functions for the RT_PREEMPT and POSIX non real-time
*               thread systems.
*
...
static int posix_task_self_hook(void);
...
static int have_cg;  // true when libcgroup initialized successfully
#endif  /* RTAPI */


#ifdef RTAPI
static int posix_module_init_hook(void)
{
    int ret;
...
#else
static int posix_module_init_hook(void) { return 0; }
#endif
...
************************************************************************/
static int posix_task_update_stats_hook(void)
{
...
static int posix_task_new_hook(task_data *task, int task_id) {
    void *stackaddr;
...
static int posix_task_delete_hook(task_data *task, int task_id) {
...
static int posix_task_start_hook(task_data *task, int task_id) {
    pthread_attr_t attr;
    int retval;
...
static int posix_task_stop_hook(task_data *task, int task_id) {
    extra_task_data[task_id].destroyed = 1;
    return 0;
}

static int posix_wait_hook(const int flags) {
    struct timespec ts;
...
static void posix_task_delay_hook(long int nsec)
{
    struct timespec t;
...
static long long posix_task_pll_get_reference_hook(void) {
    int task_id = posix_task_self_hook();
...
static int posix_task_pll_set_correction_hook(long value) {
    int task_id = posix_task_self_hook();
