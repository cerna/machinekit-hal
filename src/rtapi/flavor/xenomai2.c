...
static int xenomai_task_self_hook(void);

/***********************************************************************
*                           RT thread statistics update                *
************************************************************************/
#ifdef RTAPI
static int xenomai_task_update_stats_hook(void)
{
...
************************************************************************/
#ifdef RTAPI
static int xenomai_module_init_hook(void)
{
...
static void xenomai_module_exit_hook(void)
{
#ifdef USE_SIGXCPU
    struct sigaction sig_act;
...
#ifdef RTAPI
static int xenomai_task_delete_hook(task_data *task, int task_id)
{
    int retval = 0;
...
static void _rtapi_task_wrapper(void *task_id_hack)
{
    int ret;
...
static int xenomai_task_start_hook(task_data *task, int task_id)
{
    int which_cpu = 0;
    int uses_fpu = 0;
    int retval;
    ...
    if ((retval = rt_task_create(&ostask_array[task_id], task->name,
                                 task->stacksize, prio,
                                 uses_fpu | which_cpu | T_JOINABLE)) != 0)
    {
        rtapi_print_msg(RTAPI_MSG_ERR,
...
static int xenomai_task_stop_hook(task_data *task, int task_id)
{
    int retval;
...
static int xenomai_task_pause_hook(task_data *task, int task_id)
{
    return rt_task_suspend(&ostask_array[task_id]);
}

static int xenomai_task_resume_hook(task_data *task, int task_id)
{
    return rt_task_resume(&ostask_array[task_id]);
}

static int xenomai_wait_hook(const int flags)
{
...
#ifdef RTAPI
static void xenomai_task_delay_hook(long int nsec)
{
...
static long long int xenomai_get_time_hook(void)
{
...
*/
static long long int xenomai_get_clocks_hook(void)
{
    return rt_timer_read();
}
...
static void xenomai_print_thread_stats(int task_id)
{
...
static void xenomai_exception_handler_hook(int type,
...
#ifdef RTAPI
static flavor_descriptor_t flavor_xenomai2_descriptor = {
    .name = "xenomai2",
    .flavor_id = 4,
    .flavor_magic = 1,
...
static void lib_init(void)
{
    ...
    {
        register_flavor(flavor_xenomai2_descriptor);
    }
}
static void lib_fini(void)
{
    unregister_flavor(flavor_xenomai2_descriptor);
}
